...
# from https://github.com/jumphone/SPRINT/blob/master/sprint/sprint_main.py

import sys
from sprint import tmp as sprint
import os
import shutil
import subprocess
...
# --- rewritten (this should be its own file)
def mask_zz2snv(zz_in_dir=0, bed_out_dir=0, baseq_cutoff_dir=0, unstranded=False):
    ...
    for line in fi:
        ...
        if basequa[0] != '*' and mpqua >= limitmpqua and mpqua < 200:
            ...
            if len(baseqlst) > 0 and sum(baseqlst) / float(len(baseqlst)) >= limitbasequa:
                for snv in truesnv:
                    # Add validation for malformed mutation strings
                    if len(snv[0].split(':')) != 2:
                        print("Malformed SNV entry:", snv[0])
                        continue
                    ...
                    try:
                        ...
                        if unstranded:
                            # For unstranded, put everything in "forward" count - all will be reported as unstranded
                            allsnv[seq[0] + '\t' + snv[0].split(':')[0] + '\t' + snv[0].split(':')[1]][1] = allsnv[seq[0] + '\t' + snv[0].split(':')[0] + '\t' + snv[0].split(':')[1]][1] + 1
                        else:
                            # Original strand logic
                        ...
                        if unstranded:
                            # For unstranded, initialize with "forward" count - all will be reported as unstranded
    ...
    for snv in allsnv:
        ...
        if allsnv[snv][0] >= limitad:
            if unstranded:
                # For unstranded, always use '.' strand
                snv_bed.append([seq[0], int(seq[2]), seq[1], '.', allsnv[snv][0]])
            else:
                # Original strand assignment logic
...
def main(): 
    ...
    # function to convert fastq to sam
    def fq2sam(TAG,paired_end,read1,tmp,refgenome,bwa,samtools,mapcpu,read2=None):
        if paired_end==True:
    ...
    if 1==1: 
        ...
        # auto-detecting the FASTQ quality‐score offset
        sprint.get_baseq_cutoff(read1,tmp+'baseq.cutoff')

        print 'mapping...'

        TAG='genome'
        # first pass, map read to genome
        fq2sam(TAG,paired_end,tmp+'cut_read1.fastq',tmp,refgenome,bwa,samtools,mapcpu,tmp+'cut_read2.fastq')
    
        # pull out unmapped reads, convert to upmapped reads from both read1 and read2 to fastq
        ...
        if os.path.exists(refgenome+'.trans.fa'):
            # map reads to transcriptome, this is done in a non pair-aware way
            TAG='transcript'
            fq2sam(TAG,False,tmp+'/genome_unmapped.fq',tmp,refgenome+'.trans.fa',bwa,samtools,mapcpu)
        ...
        TAG='genome_mskAG'
        fq2sam(TAG,False,tmp+'/regular_unmapped_A_to_G.fq',tmp,refgenome+'.mskAG.fa',bwa,samtools,mapcpu)
    
        subprocess.Popen(samtools+' view -f4 '+tmp+'/'+TAG+'/all.bam > '+tmp+'/'+TAG+'_unmapped.sam',shell=True).wait()
        sprint.umsam2fq(tmp+'/'+TAG+'_unmapped.sam',tmp+'/'+TAG+'_unmapped.fq')    
    
        TAG='genome_mskTC'
        fq2sam(TAG,False,tmp+'/regular_unmapped_A_to_G.fq',tmp,refgenome+'.mskTC.fa',bwa,samtools,mapcpu)
        ...
        if os.path.exists(refgenome+'.trans.fa'):
            ...
            fq2sam(TAG,False,tmp+'/genome_mskAG_unmapped.fq',tmp,refgenome+'.trans.fa.mskAG.fa',bwa,samtools,mapcpu)
    
            TAG='transcript_mskTC'
            fq2sam(TAG,False,tmp+'/genome_mskTC_unmapped.fq',tmp,refgenome+'.trans.fa.mskTC.fa',bwa,samtools,mapcpu)
        ...
        if os.path.exists(refgenome+'.trans.fa'):
            mask_zz2snv(tmp+'transcript_mskAG_all.zz.dedup',tmp+'transcript_mskAG_all.zz.dedup.snv',tmp+'baseq.cutoff', unstranded=unstranded) 
            mask_zz2snv(tmp+'transcript_mskTC_all.zz.dedup',tmp+'transcript_mskTC_all.zz.dedup.snv',tmp+'baseq.cutoff', unstranded=unstranded) 
            mask_zz2snv(tmp+'transcript_all.zz.dedup',tmp+'transcript_all.zz.dedup.snv',tmp+'baseq.cutoff', unstranded=unstranded)
        ...
        mask_zz2snv(tmp+'genome_mskAG_all.zz.dedup',tmp+'genome_mskAG_all.zz.dedup.snv',tmp+'baseq.cutoff', unstranded=unstranded) 
        mask_zz2snv(tmp+'genome_mskTC_all.zz.dedup',tmp+'genome_mskTC_all.zz.dedup.snv',tmp+'baseq.cutoff', unstranded=unstranded) 
        mask_zz2snv(tmp+'genome_all.zz.dedup',tmp+'genome_all.zz.dedup.snv',tmp+'baseq.cutoff', unstranded=unstranded) 
