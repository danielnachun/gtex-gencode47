...
if [ "${regenerate_all}" = true ]; then
    ...
    bams_to_hyperedit=$(sed "s|^|${bam_dir}/|" <<< "$full_bam_list")
else
    # only hyperedit a bam if hyperedited reads output doesn't exist
    bams_to_hyperedit=$(grep -v -F -f <(ls "${output_dir}/hyperedit/" | sed 's|_pe\.bam$|.Aligned.sortedByCoord.out.patched.v11md.bam|') <<< "$full_bam_list" | sed "s|^|${bam_dir}/|")
fi
