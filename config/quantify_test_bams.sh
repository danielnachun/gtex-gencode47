# sample args for quantifying 3 test bams
bam_dir=/oak/stanford/groups/smontgom/dnachun/data/gtex/v10/output/test_bams/output_kate/genome_bam
reference_dir=/oak/stanford/groups/smontgom/dnachun/data/gtex/v10/data/realign_references
reference_fasta=Homo_sapiens_assembly38_noALT_noHLA_noDecoy.fasta
chr_sizes=GRCh38.chrsizes
genes_gtf=gencode.v47.genes.gtf
intervals_bed=gencode.v47.GRCh38.insert_size_intervals_geq1000bp.bed
vcf_dir=/oak/stanford/groups/smontgom/dnachun/data/gtex/v10/data/processed/vcfs_dan
output_dir=/oak/stanford/groups/smontgom/dnachun/data/gtex/v10/output/test_bams/output_kate/genome_bam
code_dir=/oak/stanford/groups/smontgom/dnachun/data/gtex/v10/scripts/processing
